...
4. Review the leading result and alternatives. You can choose a suggestion or enter the correct
   species, observation time, camera/location label, and optional notes.
5. Select **Save observation**. Nothing is added to detections, counts, or leaderboards before this
   confirmation.
...
- Uploaded observations use a `manual_…` event identity and are excluded from Frigate missing-media
  reconciliation. Existing snapshot and clip views still serve their persisted local media.
...
`observation_source` API field distinguishes `manual_upload` from `frigate`, and the original
filename is treated as display metadata rather than a filesystem path.

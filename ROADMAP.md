...
| 1. Catalogue schema and deterministic builder | 🔄 the dedicated Alembic stream, full catalogue schema, deterministic IOC seed build, marker-guarded seed-then-copy into `/data`, and the transactional release importer with rollback are delivered (single-head, reversible, CI-smoked, constraints in-schema; interrupted imports leave the previous release active; identities are never deleted; a lost catalogue is reported, never silently replaced). Remaining: the Catalogue of Life import for non-bird classes |

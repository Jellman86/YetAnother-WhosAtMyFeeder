...
- On `dev`, auto video analysis also treats classifier `Unknown` / `Unknown Bird` outputs as abstentions for frame aggregation, top-frame persistence, promotion, and completion broadcasts, preventing a high-confidence unknown analysis frame from overriding known-species evidence or steering HQ snapshot reuse.
...
- Count `unknown_top1` and `high_confidence_unknown` separately from ordinary wrong-species failures.

...
| 3. Shadow resolution and historical backfill | 🔄 live shadow resolution is delivered: detections gain nullable `species_id` / `model_artifact_id` / `model_output_index` columns by reversible migration, every live classification resolves its checksum-bound output through the catalogue beside the label path, identity persists only on agreement (recorded synonyms count), and disagreements are counted in Health shadow statistics rather than written to history. The conservative historical backfill is also delivered: rows whose scientific name resolves to exactly one identity gain `species_id` at startup, idempotently, with ambiguous and unknown names counted and left untouched. Remaining: routing audio correlation, manual observations, backfill, and video reclassification through the same resolver |

...
- On `dev`, auto video analysis also treats classifier `Unknown` / `Unknown Bird` outputs as abstentions for frame aggregation and top-frame persistence, preventing a high-confidence unknown analysis frame from overriding known-species evidence or steering HQ snapshot reuse.

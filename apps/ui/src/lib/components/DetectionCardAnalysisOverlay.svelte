...
<div class="detection-card-analysis-overlay absolute inset-0 z-20 flex flex-col justify-between rounded-3xl border border-indigo-300/35 bg-slate-950/82 p-4 text-white shadow-2xl backdrop-blur-md">
    <div class="absolute inset-0 rounded-3xl bg-gradient-to-br from-indigo-500/20 via-slate-950/20 to-cyan-500/20"></div>
    <div class="absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-white/10 to-transparent"></div>

    <div class="relative flex items-start justify-between gap-3">
        <div class="min-w-0">
            <div class="flex flex-wrap items-center gap-1.5">
                <span class="inline-flex h-2 w-2 rounded-full bg-cyan-300 {isComplete ? '' : 'animate-pulse'}"></span>
                <span class="text-[10px] font-black uppercase tracking-widest text-cyan-200">
            ...
            <p class="mt-1 text-[11px] font-semibold text-slate-200">
                {hasFallenBackToSnapshot
        ...
        <div class="shrink-0 rounded-2xl border border-white/15 bg-white/10 px-2.5 py-1.5 text-right">
            <p class="text-lg font-black leading-none text-white">{progressPercent}%</p>
            <p class="mt-0.5 text-[8px] font-black uppercase tracking-widest text-slate-300">
    ...
    <div class="relative space-y-3">
        <div class="overflow-hidden rounded-full bg-white/12">
            <div
        ...
        <div class="rounded-2xl border border-white/12 bg-black/25 p-3">
            <div class="flex items-center justify-between gap-3 text-[10px] font-black uppercase tracking-widest text-slate-300">
                ...
                {#if isAutoVideoReclassification}
                    <span class="rounded-md border border-cyan-300/30 bg-cyan-300/10 px-1.5 py-0.5 text-cyan-200">
            ...
            {#if displayLabel}
                <div class="mt-2 flex items-center justify-between gap-3">
                    <p class="min-w-0 truncate text-sm font-black text-white">{displayLabel}</p>
                    {#if displayScorePercent !== null}
                        <span class="shrink-0 text-xs font-black text-cyan-200">{displayScorePercent}%</span>
                    {/if}
                </div>
            {/if}
        </div>

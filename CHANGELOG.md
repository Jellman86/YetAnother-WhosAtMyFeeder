...
- **Fixed:** Increased the `asyncio` subprocess reader limit to 512KB. This prevents `LimitOverrunError` crashes when workers output large amounts of data at once (e.g., massive GPU error stack traces or large JSON result payloads).
- **Changed:** The Video Classifier now automatically pauses batch analysis whenever live detections are active or queuing. This prioritization frees up GPU and RAM for immediate detections and prevents OOM crashes when running heavy "Elite" models under sustained load.

...
- **Changed:** The Events explorer multi-select cards now use an edge-mounted icon-only selector instead of an in-image `Select` / `Selected` pill. Selected cards also get stronger cyan framing so bulk selection reads clearly without covering snapshot badges or bird imagery.

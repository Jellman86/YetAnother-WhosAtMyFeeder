...
- **CI:** Build workflow now triggers on both `dev` and `main` branches. `main` branch pushes produce `:main` image tags; `dev` branch pushes produce `:dev`; version tags produce `:vX.Y.Z` + `:latest` (unchanged).

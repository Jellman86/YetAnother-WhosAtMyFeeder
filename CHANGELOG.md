...
- **Added:** Leaderboard regression coverage now exercises `/api/leaderboard/species` with hidden noncanonical detections so the `Unknown Bird` branch cannot silently reintroduce the parameter-binding failure.

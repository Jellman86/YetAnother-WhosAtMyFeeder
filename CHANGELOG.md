...
- **Changed:** The Jobs page now keeps `Recent` history stable and easier to scan. Recent jobs are explicitly sorted by newest terminal event first, rows include compact job-type icons, and active job lanes now reflect the configured concurrency instead of inventing extra thread slots. If the backend ever reports more active work than visible lane capacity, the overflow is called out separately instead of fabricating additional lanes.

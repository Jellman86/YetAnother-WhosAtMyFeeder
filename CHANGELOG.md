...
- **Fixed:** Canonical identity repair now also refreshes already-populated detections whose stored common name is non-English/non-ASCII, instead of only filling missing taxonomy fields. This restores the maintenance repair action's ability to clean up degraded historical taxonomy rows without disturbing valid canonical scientific names or `taxa_id` values.

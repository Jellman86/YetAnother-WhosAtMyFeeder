...
- **Changed:** The Events explorer multi-select cards now use an icon-only selector and a much stronger selected-card treatment instead of the old in-image `Select` / `Selected` pill. Selected cards now get a bolder cyan frame plus a subtle frosted cyan overlay across the image area so bulk selection reads clearly from the grid without covering snapshot badges or bird imagery with text.

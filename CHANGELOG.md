...
- **Added:** Experimental classifier runtime toggle `CLASSIFIER_STRICT_NON_FINITE_OUTPUT` (default `true`). Setting it to `false` disables strict rejection of all-non-finite inference outputs and coerces them into normalized probabilities for controlled debugging/soak tests, while `/api/classifier/status` now exposes `strict_non_finite_output` so active policy is explicit in diagnostics.

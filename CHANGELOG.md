...
- **Changed:** `video_classification_max_concurrent` now acts as a shared maintenance-work limit, not just a video-worker limit. Detection backfill, weather backfill, analyze-unknowns, taxonomy sync, and queued maintenance video work now share the same global slot budget, defaulting to `1` for conservative behavior and preventing overlapping maintenance jobs from stacking unexpectedly. (Issue #33 / maintenance hardening)

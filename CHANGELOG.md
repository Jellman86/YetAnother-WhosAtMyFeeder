...
- **Fixed:** Video classification progress now accurately reaches 100% in the UI overlay even for short videos or when some frames are skipped due to inference errors. The frontend now correctly trusts the backend's frame total instead of sticking to the configured maximum, and progress callback signatures were hardened to prevent internal reporting mismatches.

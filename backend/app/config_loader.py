...
def load_settings_instance(settings_cls: type[Any], config_path: Path) -> Any:
    ...
    frigate_data = {
        ...
        "recording_clip_after_seconds": int(os.environ.get("FRIGATE__RECORDING_CLIP_AFTER_SECONDS", "90")),
    ...
    maintenance_data = {
        ...
        "auto_delete_missing_clips": os.environ.get("MAINTENANCE__AUTO_DELETE_MISSING_CLIPS", "false").lower()
        == "true",
    }
    ...
    classification_data = {
        ...
        "personalized_rerank_enabled": os.environ.get("CLASSIFICATION__PERSONALIZED_RERANK_ENABLED", "false").lower()
        == "true",
        "auto_video_classification": os.environ.get("CLASSIFICATION__AUTO_VIDEO_CLASSIFICATION", "false").lower()
        == "true",
        "video_classification_delay": int(os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_DELAY", "30")),
        "video_classification_max_retries": int(
            os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_MAX_RETRIES", "3")
        ),
        "video_classification_retry_interval": int(
            os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_RETRY_INTERVAL", "15")
        ),
        "video_classification_max_concurrent": int(
            os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_MAX_CONCURRENT", "1")
        ),
        "video_classification_failure_threshold": int(os.environ.get("CLASSIFICATION__VIDEO_FAILURE_THRESHOLD", "5")),
        "video_classification_failure_window_minutes": int(
            os.environ.get("CLASSIFICATION__VIDEO_FAILURE_WINDOW_MINUTES", "10")
        ),
        "video_classification_failure_cooldown_minutes": int(
            os.environ.get("CLASSIFICATION__VIDEO_FAILURE_COOLDOWN_MINUTES", "15")
        ),
        "video_classification_timeout_seconds": int(
            os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_TIMEOUT_SECONDS", "180")
        ),
        "video_classification_stale_minutes": int(
            os.environ.get("CLASSIFICATION__VIDEO_CLASSIFICATION_STALE_MINUTES", "15")
        ),
        ...
        "worker_breaker_cooldown_seconds": float(
            os.environ.get("CLASSIFICATION__WORKER_BREAKER_COOLDOWN_SECONDS", "60.0")
        ),
        "live_event_stale_drop_seconds": float(os.environ.get("CLASSIFICATION__LIVE_EVENT_STALE_DROP_SECONDS", "30.0")),
        "live_event_coalescing_enabled": os.environ.get("CLASSIFICATION__LIVE_EVENT_COALESCING_ENABLED", "true").lower()
        == "true",
    ...
    media_cache_data = {
        ...
        == "true",
        "high_quality_event_snapshot_bird_crop": os.environ.get(
            "MEDIA_CACHE__HIGH_QUALITY_EVENT_SNAPSHOT_BIRD_CROP", "false"
        ).lower()
        == "true",
    ...
    # Location settings
    location_data = {"latitude": None, "longitude": None, "automatic": True, "weather_unit_system": "metric"}
    ...
    enrichment_data = {
        ...
        "links_sources": [
            s.strip()
            for s in os.environ.get("ENRICHMENT__LINKS_SOURCES", "wikipedia,inaturalist").split(",")
            if s.strip()
        ],
    }
    ...
    llm_data = {
        ...
        "model": os.environ.get("LLM__MODEL", DEFAULT_LLM_MODEL),
        "analysis_prompt_template": os.environ.get("LLM__ANALYSIS_PROMPT_TEMPLATE", DEFAULT_AI_ANALYSIS_PROMPT),
        "conversation_prompt_template": os.environ.get(
            "LLM__CONVERSATION_PROMPT_TEMPLATE", DEFAULT_AI_CONVERSATION_PROMPT
        ),
    ...
    telemetry_data = {
        ...
        "health_url": os.environ.get(
            "TELEMETRY__HEALTH_URL", "https://yawamf-telemetry.ya-wamf.workers.dev/health-issues"
        ),
    }
    ...
    auth_data = {
        "initial_setup_complete": os.environ.get("AUTH__INITIAL_SETUP_COMPLETE", "false").lower() == "true",
    ...
    if config_path.exists():
        try:
            ...
            if "classification" in file_data:
                ...
                if "inference_provider" not in cls_file and "use_cuda" in cls_file:
    ...
    if "APPEARANCE__COLOR_THEME" not in os.environ and not appearance_data.get("color_theme_default_migrated"):
    ...
    log.info(
        "MQTT config",
        server=frigate_data["mqtt_server"],
        port=frigate_data["mqtt_port"],
        auth=frigate_data["mqtt_auth"],
    )
    ...
    log.info(
        ...
        blocked_species=classification_data.get("blocked_species", []),

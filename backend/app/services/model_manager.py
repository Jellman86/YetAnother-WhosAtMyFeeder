...
REMOTE_REGISTRY = [
    ...
    {
        "id": "convnext_v2_tiny_eu_common",
        "name": "ConvNeXt-V2 Tiny EU-Common (iGPU)",
        "description": "ConvNeXt-V2-Tiny on Birder's eu-common dataset (707 bird species). Same architecture family as medium_birds_eu and validated to work cleanly on Intel iGPU with OpenVINO f32 hint. Wider EU/migrant vocabulary than the medium_birds family.",
        ...
        "recommended_for": "EU users with Intel iGPU. Wider species coverage than medium_birds at GPU latency.",
        "estimated_ram_mb": 512,
        "advanced_only": True,
        "sort_order": 21,
        "status": "experimental",
        "crop_generator": {"enabled": True},
        "notes": "Sourced from huggingface.co/birder-project/convnext_v2_tiny_eu-common. Converted via legacy torch.onnx.export (opset 14) — the dynamo path emits SequenceEmpty/ConcatFromSequence ops OpenVINO can't load. Empirically validated on Intel iGPU 2026-05-08: top-1 matches CPU on real bird image, range_ratio 1.28."
    },
    {

...
async def test_audio_confirmation_accepts_audio_name_variants_via_taxonomy(mock_dependencies):
    ...
    classifier.classify_async_live = AsyncMock(
        return_value=[{"label": "Common Wood-Pigeon", "score": 0.83, "index": 1}]
    )

    mock_dependencies["det_service"].filter_and_label.return_value = (
        {"label": "Common Wood-Pigeon", "score": 0.83},
        {},
    )

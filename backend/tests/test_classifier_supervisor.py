...
@pytest.mark.asyncio
async def test_classifier_supervisor_abort_request_replaces_matching_worker():
    created: list[_FakeWorker] = []
    ...
    with pytest.raises(ClassifierWorkerDeadlineExceededError, match="worker aborted after lease expiry"):
        await task
    assert created[0].killed is True
    assert supervisor.get_metrics()["live"]["restarts"] == 1
...
@pytest.mark.asyncio
async def test_classifier_supervisor_abort_request_ignores_stale_token():
    created: list[_FakeWorker] = []
    ...
    await created[0].events.put(
        {
            ...
            "work_id": "live-stale",
            "lease_token": 8,
            "results": [{"label": "Robin", "score": 0.88}],
        }
    )
...
@pytest.mark.asyncio
async def test_classifier_supervisor_allows_background_deadline_override():
    created: list[_FakeWorker] = []
    ...
    supervisor = ClassifierSupervisor(
        ...
        heartbeat_timeout_seconds=5.0,
        hard_deadline_seconds=0.05,
        background_hard_deadline_seconds=0.5,
        worker_factory=_factory,
        watchdog_interval_seconds=0.01,
    )
    ...
    task = asyncio.create_task(
        supervisor.classify(
            priority="background",
            work_id="background-deadline-override",
            lease_token=1,
            image_b64="payload",
            camera_name="garden",
            model_id="default",
    ...
    await worker.events.put(
        {
            "type": "result",
            "worker_generation": 1,
            "request_id": worker.sent_messages[0]["request_id"],
            "work_id": "background-deadline-override",
            "lease_token": 1,
            "results": [{"label": "Robin", "score": 0.91}],
        }

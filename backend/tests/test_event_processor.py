...
async def test_process_mqtt_message_valid_bird():
    ...
    with (
        ...
        mock_taxonomy.get_names = AsyncMock(
            return_value={"scientific_name": "Cardinalis cardinalis", "common_name": "Northern Cardinal"}
        )
        ...
        payload = (
            b'{"after": {"id": "123", "label": "bird", "camera": "cam1", "start_time": 1700000000, "end_time": null}}'
        )
...
async def test_process_mqtt_message_does_not_trigger_auto_full_visit_for_new_events():
    ...
    new_payload = (
        b'{"type":"new","after":{"id":"evt-new-no-full-visit","label":"bird","camera":"cam1","start_time":1700000000}}'
    )
    await processor.process_mqtt_message(new_payload)
...
@pytest.mark.asyncio
async def test_end_event_recovers_detection_when_initial_ingest_created_no_row():
    processor = EventProcessor(MagicMock())
    processor.detection_service.get_detection_by_frigate_event = AsyncMock(return_value=None)
    processor._classify_snapshot = AsyncMock(
        return_value=([{"label": "Cardinal", "score": 0.9, "index": 1}], b"img", "frigate_snapshot_cropped")
    ...
    payload = (
        b'{"type":"end","after":{"id":"evt-end-recovery","label":"bird","camera":"cam1",'
...
@pytest.mark.asyncio
async def test_terminal_enrichment_isolates_media_queue_failures_after_durable_detection():
    processor = EventProcessor(MagicMock())
    event = SimpleNamespace(
        frigate_event="evt-terminal-side-effects",
        camera="cam1",
        start_time_ts=1700000000,
        end_time_ts=1700000005,
        data={},
        snapshot=None,
    )
    processor._auto_full_visit_enabled = MagicMock(return_value=True)  # type: ignore[method-assign]
    processor._trigger_auto_full_visit_generation = AsyncMock(  # type: ignore[method-assign]
        side_effect=RuntimeError("full")
    )

    with (
        patch("app.services.event_processor.media_cache.has_snapshot", return_value=True),
        patch(
            "app.services.event_processor.high_quality_snapshot_service.schedule_final_replacement",
            side_effect=RuntimeError("full"),
        ),
    ):
        await processor._handle_terminal_event_enrichment(event)

    processor._trigger_auto_full_visit_generation.assert_awaited_once_with(event)
...
@pytest.mark.asyncio
async def test_post_commit_side_effect_failure_does_not_turn_saved_detection_into_pipeline_failure():
    processor = EventProcessor(MagicMock())
    event = SimpleNamespace(
        frigate_event="evt-post-commit",
        camera="cam1",
        start_time_ts=1700000000,
        frigate_score=0.95,
        sub_label=None,
    ...
    with (
        patch(
            "app.services.event_processor.frigate_client.set_sublabel",
...
async def test_handle_detection_save_and_notify_schedules_high_quality_snapshot_replacement():
    ...
    with (
        ...
        mock_cache.cache_snapshot.assert_awaited_once_with(
            "evt-hq-1",
            b"img",
...
async def test_process_mqtt_message_logs_stage_timeout_for_classification():
    ...
    with (
        patch("app.services.event_processor.log") as mock_log,
...
async def test_process_mqtt_message_caps_live_queue_timeout_to_remaining_freshness():
    ...
    with (
        ...
        event_received_at = 1700000001.0 - 29.0
        payload = {
...
def test_live_queue_timeout_still_caps_to_remaining_freshness_under_pressure():
    ...
    processor = EventProcessor(classifier)
    event = SimpleNamespace(received_at_ts=1700000001.0 - 29.0)
...
async def test_process_mqtt_message_preserves_live_overload_from_classifier_service():
    ...
    with (
        ...
        payload = (
            b'{"type":"new","after":{"id":"evt-overload-live","label":"bird","camera":"cam1","start_time":1700000000}}'
        )
        await processor.process_mqtt_message(payload)
...
async def test_process_mqtt_message_records_live_lease_expiry_as_timeout_drop():
    ...
    with (
        ...
        payload = (
            b'{"type":"new","after":{"id":"evt-lease-timeout","label":"bird","camera":"cam1","start_time":1700000000}}'
        )
        await processor.process_mqtt_message(payload)
...
async def test_process_mqtt_message_audio_taxonomy_lookup_timeout_falls_back():
    ...
    payload = (
        b'{"type":"new","after":{"id":"evt-audio-tax-timeout","label":"bird","camera":"cam1","start_time":1700000000}}'
    )
...
async def test_process_mqtt_message_status_tracks_completed_event():
    ...
    processor = EventProcessor(classifier)
    processor._classify_snapshot = AsyncMock(  # type: ignore[method-assign]
        return_value=([{"label": "Sparrow", "score": 0.95, "index": 1}], b"img", "frigate_snapshot_cropped")
    )
    processor._gather_context_data = AsyncMock(  # type: ignore[method-assign]
        return_value={"audio_match": None, "weather_data": {}}
    )
...
async def test_process_mqtt_message_coalesces_duplicate_live_event_while_active():
    ...
    payload = (
        b'{"type":"new","after":{"id":"evt-coalesce-live-1","label":"bird","camera":"cam1","start_time":1700000000}}'
    )
...
async def test_process_mqtt_message_allows_retry_after_classification_when_first_save_is_blocked():
    ...
    payload = (
        b'{"type":"new","after":{"id":"evt-save-retry-live-1","label":"bird","camera":"cam1","start_time":1700000000}}'
    )
...
def test_event_processor_status_recovers_after_stale_critical_failure():
    ...
    with (
        patch("app.services.event_processor.time.monotonic", return_value=20.0),
...
async def test_classify_snapshot_extends_retry_budget_during_frigate_stall_recovery():
    ...
    assert snapshot_source == "frigate_snapshot_cropped"
...
async def test_classify_snapshot_falls_back_to_uncropped_snapshot_when_cropped_missing():
    ...
    assert snapshot_source == "frigate_snapshot_uncropped"
...
async def test_classify_snapshot_falls_back_to_thumbnail_before_drop():
    ...
    assert snapshot_source == "frigate_thumbnail"
    mock_cache.get_snapshot.assert_not_awaited()
    mock_sleep.assert_awaited_once()
...
async def test_classify_snapshot_falls_back_to_cached_snapshot_before_drop():
    ...
    with (
        ...
        mock_cache.get_snapshot_metadata = AsyncMock(return_value={"source": "high_quality_bird_crop"})
        mock_mqtt.get_status.return_value = {
            "last_reconnect_reason": None,
            "intentional_reconnect_pending": False,
            "stall_recovery_warning_active": False,
    ...
    assert snapshot_source == "high_quality_bird_crop"
    assert classifier.classify_async_live.await_args.kwargs["input_context"] == {
        "is_cropped": True,
        "event_id": "evt-fallback-cache",
        "input_source": "high_quality_bird_crop",
    }
...
@pytest.mark.asyncio
async def test_classify_snapshot_does_not_claim_crop_for_ended_frigate_event():
    classifier = MagicMock()
    classifier.classify_async_live = AsyncMock(return_value=[{"label": "Sparrow", "score": 0.95, "index": 1}])
    processor = EventProcessor(classifier)
    event = SimpleNamespace(
        frigate_event="evt-ended",
        camera="cam1",
        sub_label=None,
        frigate_score=None,
        is_false_positive=False,
    ...
    with (
        ...
        mock_frigate.get_snapshot_with_error = AsyncMock(return_value=(b"saved-snapshot", None))

        result = await processor._classify_snapshot(event)

    assert result is not None
    _, snapshot_data, snapshot_source = result
    assert snapshot_data == b"saved-snapshot"

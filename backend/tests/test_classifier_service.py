...
@pytest.mark.asyncio
async def test_register_gpu_unhealthy_signal_does_not_double_count_live_lease_expiry(
    mock_tflite, mock_os_path_exists, monkeypatch
):
    with patch.object(ClassifierService, "_init_bird_model", return_value=None):
        monkeypatch.setattr(
        ...
        service.register_gpu_unhealthy_signal("live_lease_expiry", event_id="evt-one-lease")

        assert service._inference_health.verdict(runtime_key) == "healthy"
...
@pytest.mark.asyncio
async def test_classifier_service_skips_crop_when_input_is_already_cropped(mock_tflite, mock_os_path_exists):
    class _CropAwareBirdModel:
        def __init__(self):
            self.loaded = True
            self.error = None
...
@pytest.mark.asyncio
async def test_classifier_service_skips_crop_when_manifest_disables_it(mock_tflite, mock_os_path_exists):
    class _CropAwareBirdModel:
        def __init__(self):
            self.loaded = True
            self.error = None
...
@pytest.mark.asyncio
async def test_classifier_service_falls_back_to_original_image_when_crop_service_returns_no_crop(
    mock_tflite, mock_os_path_exists
):
    class _CropAwareBirdModel:
        def __init__(self):
...
async def test_classifier_service_prefers_high_quality_source_before_crop_generation(
    ...
    class _FakeCropService:
        ...
        def generate_crop(self, image):
            self.calls.append(image)
            return {
                "crop_image": Image.new("RGB", (12, 12), color="green"),
                "box": (1, 1, 13, 13),
                "confidence": 0.88,
                "reason": "selected",
            }
    ...
    with patch.object(ClassifierService, "_init_bird_model", return_value=None), \
        ...
        assert results[0]["label"] == "Robin"
        assert crop_source_resolver.calls[0]["image"] is image
        assert crop_source_resolver.calls[0]["source_preference"] == "high_quality"
        assert crop_service.calls[0].size == (48, 48)
        assert bird_model.seen_images[0].size == (12, 12)
        await service.shutdown()


@pytest.mark.asyncio
async def test_classifier_service_falls_back_to_original_image_when_high_quality_source_resolution_fails(
    mock_tflite, mock_os_path_exists
):
    class _CropAwareBirdModel:
        def __init__(self):
    ...
    class _ExplodingCropSourceResolver:
        def __init__(self):
            self.calls = []
    ...
    with patch.object(ClassifierService, "_init_bird_model", return_value=None), \
        ...
        bird_model = _CropAwareBirdModel()
        crop_service = _FakeCropService()
        crop_source_resolver = _ExplodingCropSourceResolver()
        service._models["bird"] = bird_model
        service._bird_crop_service = crop_service
        service._crop_source_resolver = crop_source_resolver
...
async def test_classifier_service_can_prefer_crop_model_before_frigate_hint(
    ...
    with patch.object(ClassifierService, "_init_bird_model", return_value=None), \
        ...
        assert results[0]["label"] == "Robin"
        assert len(crop_service.calls) == 1
        assert crop_service.calls[0].size == (400, 400)
        assert bird_model.seen_images[0].size == (18, 20)
        await service.shutdown()
